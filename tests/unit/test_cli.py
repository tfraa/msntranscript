...
from msnpip.cli import build_parser, main  # noqa: E402
...
class TestArgParsing:
    def test_method_and_enrichment_append(self):
        args = build_parser().parse_args([
            "full", "--dataframe", "m.csv", "--output", "o",
            "--method", "pls", "--method", "corr",
            "--enrichment", "ensemble", "--enrichment", "gsea",
        ])
...
class TestFullRun:
    def test_full_run_writes_report(self, tmp_path, monkeypatch):
        ...
        rc = main([
            "full", "--dataframe", str(info["merged_path"]), "--output", str(out),
            "--group-col", "group", "--case", "FTD", "--control", "HC",
            "--predictors", "age", "sex", "--method", "pls", "--ncomp", "1",
            "--enrichment", "ensemble", "--n-perm", "10",
        ])
        assert rc == 0
    ...
    def test_unknown_atlas_returns_1(self, tmp_path):
        rc = main([
            "full", "--dataframe", "m.csv", "--output", str(tmp_path / "o"),
            "--group-col", "group", "--case", "A", "--control", "B",
            "--atlas", "definitely_not_an_atlas",
        ])
        assert rc == 1
